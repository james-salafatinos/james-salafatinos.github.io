...
const texture = loader.load(
  "https://cdn.glitch.com/4ce94fe8-ed51-4ffd-be03-fb92308989d7%2Fdownload.jpg?v=1630059706571"
);
const sphereTexture = loader.load(
  "https://cdn.glitch.com/4ce94fe8-ed51-4ffd-be03-fb92308989d7%2Fpanel%2B023.jpgb2eb29dc-b2c3-4ac7-b9fc-aced0370ab0dLarger.jpg?v=1628445916919"
);
const displace = loader.load(
  "https://cdn.glitch.com/4ce94fe8-ed51-4ffd-be03-fb92308989d7%2Fcity%2Bdepth%2Bmap.jpg?v=1628446293385"
);
const sphereNormal = loader.load(
  "https://cdn.glitch.com/4ce94fe8-ed51-4ffd-be03-fb92308989d7%2FXsfXDr.jpg?v=1628448132617"
);
const alpha = loader.load(
  "https://cdn.glitch.com/4ce94fe8-ed51-4ffd-be03-fb92308989d7%2Fphoto-1621623681348-8ef41a7060ae.jpg?v=1628445985831"
);
...
let shoot_basic = function (dirVector) {
  let geo = new THREE.SphereBufferGeometry(0.2);
  let mat = new THREE.MeshPhongMaterial();
  let mesh = new THREE.Mesh(geo, mat);
  mesh.position.x = camera.position.x;
  mesh.position.y = camera.position.y;
  mesh.position.z = camera.position.z;
  mesh.userData.dirVector = dirVector.multiplyScalar(0.2);
...
var top = createPlaneFace(0, spacing, 0, 0.5, 1, 1, 0, 1, 0);
scene.add(top); //constructRacquetballRoom()

var bottom = createPlaneFace(0, -spacing, 0, 0.5, 1, 1, 0, -1, 0);
...
var left2 = createPlaneFace(spacing, 0, 0, 1, 0.5, 1, -1, 0, 0);
scene.add(left2);
var right2 = createPlaneFace(-spacing, 0, 0, 1, 0.5, 1, 1, 0, 0);
scene.add(right2);
...
function collisionVector(v1, v2, x1, x2) {
  let d = v1.clone().sub(
    x1
      ...
      .sub(x2.clone())
      .multiplyScalar(
        v1.clone().sub(v2.clone()).dot(x1.clone().sub(x2.clone())) /
...
function planeCollisionVector(_v1, planeMesh) {
  ...
  let planeNormal = new THREE.Vector3(
    planeMesh.userData.normal.x,
    planeMesh.userData.normal.y,
    planeMesh.userData.normal.z
  );
  let d = v1
    .clone()
    .sub(planeNormal.multiplyScalar(2 * v1.clone().dot(planeNormal.clone())));
  return d;
}
  ...
  for (let i = 0; i < balls_in_play.length; i++) {
    ...
    for (let j = i; j < balls_in_play.length; j++) {
      ...
      if (i == j) {
        ...
        if (((_x - x) ** 2 + (_y - y) ** 2 + (_z - z) ** 2) ** 0.5 < 2 * eps) {
          let ball = balls_in_play[i].clone();
          let ball_vector = new THREE.Vector3(
            ball.userData.dirVector.x,
            ball.userData.dirVector.y,
            ball.userData.dirVector.z
          );
          let ball_pos = new THREE.Vector3(
            ball.position.x,
            ball.position.y,
            ball.position.z
          );
          ...
          let ball2_vector = new THREE.Vector3(
            ball2.userData.dirVector.x,
            ball2.userData.dirVector.y,
            ball2.userData.dirVector.z
          );
          let ball2_pos = new THREE.Vector3(
            ball2.position.x,
            ball2.position.y,
            ball2.position.z
          );

          let delta1 = collisionVector(
            ball_vector,
            ball2_vector,
            ball_pos,
            ball2_pos
          );
          let delta2 = collisionVector(
            ball2_vector,
            ball_vector,
            ball2_pos,
            ball_pos
          ...
          balls_in_play[i].material.color = new THREE.Color(
            `hsl(${2000 - delta1.length() * 1000}, 100%, 50%)`
          );
          balls_in_play[j].material.color = new THREE.Color(
            `hsl(${2000 - delta2.length() * 1000}, 100%, 50%)`
          );
        } else {
          //Do nothing
        }

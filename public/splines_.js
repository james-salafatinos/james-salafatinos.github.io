...
let drawSpline = function () {
  if (globalCubeCoords.length % 2 == 0) {
    ...
    let interim_spline = new THREE.CatmullRomCurve3([
      u,
      u.clone().add(rm),
      v.clone().add(rn),
      v,
    ]);
    console.log("ArcLength", interim_spline.getPointAt(0.5));

    let _x = interim_spline.getPointAt(0.5).x;
    let _y = interim_spline.getPointAt(0.5).y;
    let _z = interim_spline.getPointAt(0.5).z;
